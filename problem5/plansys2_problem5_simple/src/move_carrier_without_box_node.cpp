...
class MoveCarrierWithoutBoxAction : public plansys2::ActionExecutorClient
{
public:
  // Each 250ms the method do_work() of the class MoveAction() is called
  MoveCarrierWithoutBoxAction() : plansys2::ActionExecutorClient("move_carrier_without_box", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Move carrier without box in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Move carrier without box completed");
    ...
    std::cout << "Moving carrier without box ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<MoveCarrierWithoutBoxAction>();

  // Define the name of the action node
  node->set_parameter(rclcpp::Parameter("action_name", "move_carrier_without_box"));
