...
class UnfillBoxAction : public plansys2::ActionExecutorClient
{
public:
  // Each 250ms the method do_work() of the class MoveAction() is called
  UnfillBoxAction() : plansys2::ActionExecutorClient("unfill_box", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    ...
    std::cout << "Unfilling box ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<UnfillBoxAction>();

  // Define the name of the action node
  node->set_parameter(rclcpp::Parameter("action_name", "unfill_box"));
