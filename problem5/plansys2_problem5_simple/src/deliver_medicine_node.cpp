...
class DeliverMedicineAction : public plansys2::ActionExecutorClient
{
public:
  // Each 250ms the method do_work() of the class MoveAction() is called
  DeliverMedicineAction() : plansys2::ActionExecutorClient("deliver_medicine", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Deliver medicine in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Deliver medicine completed");
    ...
    std::cout << "Delivering medicine ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<DeliverMedicineAction>();

  // Define the name of the action node
  node->set_parameter(rclcpp::Parameter("action_name", "deliver_medicine"));
