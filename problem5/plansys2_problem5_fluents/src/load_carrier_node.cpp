...
class LoadCarrierAction : public plansys2::ActionExecutorClient
{
public:
  // Each 250ms the method do_work() of the class MoveAction() is called
  LoadCarrierAction() : plansys2::ActionExecutorClient("load_carrier", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Load carrier in progress");
    } else {
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<LoadCarrierAction>();
