...
class MoveCarrierWithBoxAction : public plansys2::ActionExecutorClient
{
public:
  // Each 250ms the method do_work() of the class MoveAction() is called
  MoveCarrierWithBoxAction() : plansys2::ActionExecutorClient("move_carrier_with_box", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Move carrier with box in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Move carrier with box completed");
    ...
    std::cout << "Moving carrier with box ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<MoveCarrierWithBoxAction>();

  // Define the name of the action node
  node->set_parameter(rclcpp::Parameter("action_name", "move_carrier_with_box"));
